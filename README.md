...
| Nguyễn Đình Bảo Long | 2A202600981 | Golden dataset và evaluation pipeline | Done |
| Phạm Hoài Nam | 2A202600954 | A/B evaluation, results report, README kiến trúc | Done |
...
- `golden_dataset.json`: 19 Q&A grounded trong corpus legal/news.
- `eval_pipeline.py`: entrypoint chạy evaluation, chọn DeepEval là primary framework.
- `evaluation_rag_runner.py`: adapter gọi RAG theo config A/B.
- `evaluation_metrics.py`: metric fallback offline gồm faithfulness, answer relevance, context recall, context precision.
- `evaluation_report.py`: export Markdown report.
- `results.md`: báo cáo đã sinh với A/B comparison `hybrid_rerank` vs `hybrid_no_rerank`.

Mặc định repo chạy được offline và ghi rõ `local_heuristic_fallback` trong report. Nếu có môi trường DeepEval/LLM judge, bật `DEEPEVAL_ENABLE_REMOTE=1` để dùng DeepEval scoring.
...
- Evaluation nhóm đã có 19 golden Q&A, 4 metrics, A/B comparison và report thật.
